...
typedef void (*_pfnSDL_GL_SwapWindow)(SDL_Window*);
static _pfnSDL_GL_SwapWindow pSDL_GL_SwapWindow;

static funchook_t *pHook;

void HL_ImGUI_Draw(SDL_Window* window);

// To draw imgui on top of Half-Life, we hook SDL_GL_SwapWindow using funchook
void HL_ImGUI_Init() {

	pHook = funchook_create();
	HMODULE hSdl2 = LoadLibrary("SDL2.dll");
	...
	pSDL_GL_SwapWindow = (_pfnSDL_GL_SwapWindow)(GetProcAddress(hSdl2, "SDL_GL_SwapWindow"));
	funchook_prepare(pHook, (void**)&pSDL_GL_SwapWindow, HL_ImGUI_Draw);
	funchook_install(pHook, 0);
}
